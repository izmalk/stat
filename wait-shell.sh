#!/bin/bash
# Classic polling implementation of wait_idle.
#
# Polls `juju status --format=json` on a fixed interval and checks every
# unit's workload + agent status using an embedded Python snippet.
#
# This is the default implementation (STAT_WAIT_IMPL=shell or unset).
